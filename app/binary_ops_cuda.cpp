/**
 * examples_binary_cuda.cpp
 *
 * Runnable usage examples for binary operations on CUDA.
 * Mirrors examples_binary_cpu.cpp exactly, with device swapped to CUDA.
 */
...
void binary_add_sub()
{
    ...
    Tensor a({2, 3}, ScalarType::Float32, CUDA);
    a.fill_arange(1.0, 1.0);    // 1, 2, 3, 4, 5, 6
    print("a", a);

    Tensor b({2, 3}, ScalarType::Float32, CUDA);
...
void binary_exp()
{
    ...
    Tensor base({2, 2}, ScalarType::Float32, CUDA);
    base.fill_arange(1.0, 1.0);   // 1, 2, 3, 4
    print("base", base);

    // Exponents: 0, 1, 2, 3
    Tensor exponent({2, 2}, ScalarType::Float32, CUDA);
...
void binary_chained()
{
    ...
    Tensor c({2, 3}, ScalarType::Float32, CUDA);
...
void binary_inplace_accumulate()
{
    ...
    Tensor acc({3, 3}, ScalarType::Float32, CUDA);
    acc.fill_arange(0.0, 1.0);   // 0 … 8
    print("initial acc", acc);

    Tensor ones({3, 3}, ScalarType::Float32, CUDA);
    ones.fill_linspace(1.0, 1.0);   // all 1s
    print("ones", ones);

    Tensor scale({3, 3}, ScalarType::Float32, CUDA);
